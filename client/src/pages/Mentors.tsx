...
export default function Mentors() {
  ...
  const filteredMentors = useMemo(() => {
    ...
    if (sortBy === "rating") {
      ...
      sorted.sort((a: any, b: any) => (b.consultationCount || 0) - (a.consultationCount || 0));
    }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="bg-white border border-gray-200 rounded-full shadow-sm hover:shadow-md transition-shadow duration-300 p-2 flex items-center gap-0 flex-1">
              <Search className="h-4 sm:h-5 w-4 sm:w-5 text-gray-400 flex-shrink-0 ml-3 sm:ml-4" />
              <input
                type="text"
                ...
                className="flex-1 bg-transparent text-xs sm:text-sm placeholder-gray-400 focus:outline-none px-2 py-2 sm:px-4 sm:py-3"
              />
            </div>
            ...
            <div className="flex items-center gap-2 overflow-x-auto pb-2">
              ...
              <button
                ...
                className="flex items-center justify-center gap-1 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors border border-gray-200 whitespace-nowrap"
              >
                <span>학과</span>
              ...
              <Select value={sortBy} onValueChange={setSortBy}>
                <SelectTrigger className="w-auto border-gray-200 text-xs sm:text-sm whitespace-nowrap">
              ...
              <button
                ...
                className="flex items-center justify-center px-3 sm:px-4 py-2 rounded-lg bg-green-600 hover:bg-green-700 transition-colors text-white text-xs sm:text-sm font-medium whitespace-nowrap"
              >
                ...
                {filteredMentors.map((mentor: any) => (
                  <Link key={mentor.uuid || mentor.id} href={`/mentor/${mentor.uuid}`} className="group">
                    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg hover:border-gray-300 transition-all duration-300 h-full flex flex-col cursor-pointer">
                      ...
                      <div className="p-4 sm:p-6 border-b border-gray-100">
                        <div className="flex items-start justify-between mb-3">
                          ...
                          {mentor.verificationStatus === "approved" && (
                            <BadgeCheck className="h-5 w-5 text-green-600 flex-shrink-0 ml-2" />
                          )}
                        </div>
                        {mentor.bio && (
                      ...
                      {/* 멘토 통계 */}
                      <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-gray-100 flex items-center justify-between text-xs sm:text-sm">
                        <div className="flex items-center gap-4">
                          {mentor.averageRating && (
                            <div className="flex items-center gap-1">
                              <Star className="h-4 w-4 text-yellow-400 fill-yellow-400" />
                              <span className="font-semibold">{mentor.averageRating.toFixed(1)}</span>
                              <span className="text-gray-500">({mentor.reviewCount || 0})</span>
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {/* 지역 정보 */}
                      <div className="px-4 sm:px-6 py-3 sm:py-4 flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                        <MapPin className="h-4 w-4 flex-shrink-0" />
                        <span>{REGIONS.find(r => r.value === mentor.region)?.label || mentor.region}</span>
                      </div>

                      {/* 하단 CTA */}
                      <div className="mt-auto px-4 sm:px-6 py-3 sm:py-4 bg-gray-50 border-t border-gray-100">
                        <button className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white text-xs sm:text-sm font-medium transition-colors">
                          <span>상담 신청</span>
