...
/**
 * 사용자별 상담 시작 기록
 * 양쪽 모두 시작했을 때 상담 상태를 in_progress로 변경
 */
export async function recordUserStart(
  ...
  // 권한 검증
  if (userId !== b.studentId && userId !== b.mentorId) {
  ...
  if (userId === b.studentId) {
    updateData.studentStartedAt = new Date();
  } else {
    updateData.mentorStartedAt = new Date();
  }
  ...
  if (
    updatedBooking[0].studentStartedAt &&
    updatedBooking[0].mentorStartedAt &&
    updatedBooking[0].status === "confirmed"
  ) {
    // 양쪽 모두 시작했으면 상담 상태를 in_progress로 변경
    await db
      .update(bookings)
      .set({
        status: "in_progress",
...
/**
 * 사용자별 상담 종료 기록
 * 양쪽 모두 종료했을 때 상담 상태를 completed로 변경
 */
export async function recordUserEnd(
  bookingId: number,
  userId: number,
  endReason?: string,
  ...
  // 상태 검증
  if (b.status !== "in_progress") {
    throw new Error("Only in-progress bookings can be ended");
  }

  // 사용자별 종료 시간 기록
  const updateData: any = {};
  if (userId === b.studentId) {
    updateData.studentEndedAt = new Date();
  } else {
    updateData.mentorEndedAt = new Date();
  }

  // 첫 번째 종료 요청일 때만 종료 사유 저장
  if (!b.endReason && endReason) {
    updateData.endReason = endReason;
    updateData.endReasonDetails = endReasonDetails;
  }
  ...
  if (
    updatedBooking[0].studentEndedAt &&
    updatedBooking[0].mentorEndedAt &&
    updatedBooking[0].status === "in_progress"
  ) {
    // 양쪽 모두 종료했으면 상담 상태를 completed로 변경
    await db
      .update(bookings)
      .set({
        status: "completed",
